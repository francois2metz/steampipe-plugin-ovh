...
func createMockQueryData(t *testing.T, client *ovh.Client, projectId string, configRegions []string) *plugin.QueryData {
	// Create a mock connection with config. Connection.config is unexported and guarded by a
	// mutex, so it has to go through SetConfig rather than a struct literal.
